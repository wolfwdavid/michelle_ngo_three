...
- [x] **Lighthouse `/` LCP gate flipped warn→error (D-12 blocking pre-cutover posture).** DONE 2026-06-11 (quick task 260610-vu7): app.html hero-poster preload hoisted ahead of the JS modulepreload manifest + `createHeroDefer` eased 1000→2500ms (commit 5c1fa98); CI run 27321489171's Lighthouse job (3 runs, mobile Slow-4G, deployed staging URL) passed the 2500ms assertion clean — the earlier ~2806ms (07-03) / 2866ms (local vu7) readings were measurement-host inflation. Gate flipped to `["error", …]` in `lighthouserc.json` + deploy.yml lighthouse job renamed to blocking posture.
